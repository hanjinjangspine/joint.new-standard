(function () {
  ...
  document.addEventListener("click", function (event) {
    var element = event.target && event.target.closest ? event.target.closest("a,button,[role='button'],[data-analytics-event]") : null;
    if (!element) return;
    var eventName = classify(element);
    if (eventName) send(eventName, element);
  }, true);
